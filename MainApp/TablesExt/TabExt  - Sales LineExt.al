tableextension 50100 "Curso Tabla Extension" extends "Sales Line"
{
    ...
    begin
        ...
        CourseEdition.SetLoadFields("Max. Students", "Sales (Qty.)"); //para leer solo el campo que interesa, mexestudents
        ...
        CourseEdition.CalcFields("Sales (Qty.)"); ////llamamos a esa ufncuion calfilds e indicamos los campos que quermos calcular
 ...
 */
        //Sustitucion de logica anterior -> ////...


        ////if CourseLedgerEntry.Quantity + Rec.Quantity > CourseEdition."Max. Students" then //si la suma de CourseLedgerEntry.Quantity y el registro de quantity es mas grande que el numero maximo de estudiadntes de la edicion, manda mensaje
        ...
        if CourseEdition."Sales (Qty.)" + Rec.Quantity > CourseEdition."Max. Students" then  //// usamos el campo "Sals(Qty.) para ver los movs." de la tabla courseEdition , en vez de los filtros anteriores, se usa la formual que ya tiene ese campo creada
            Message(MaxStudentsExceededMsg, Rec."No.", Rec."Course Edition", CourseEdition."Max. Students")
    end;
}